...
- **BinaryInputMaskWithSafeWindow** (`core.interp.gx_array_view_interp.rs`)
  ...
  - Added specitic rust and python prefiltering "companions" method to update the safe window accordingly to the mask erosion based on the computed influence radius.
  - Use the previous mentioned method in python core and chain resampling method to update the safe window after Bspline prefiltering.


#### Array Window Utilities (`array_window.py`)

- **Complementary window indexing**
  - Added ``complementary_window_indices`` to compute the list of index
    tuples covering the complement of a window within an array shape
    (i.e. the padded border zone excluding the data zone). Used by
    ``apply_mask_strategy_chain`` to fill the padded zone of a
    pre-allocated mask buffer without reallocating.
  - Added unit tests for both functions covering nominal cases, edge
    windows, full-array windows, and axis-restricted variants.
...
- **Dive into grid resampling core standalone preprocessing**
  - Added the `grid_resampling_core_standalone.ipynb` notebook
  - Added the `grid_resampling_plot_utils.py` helper python module

#### Testing
- **Regression tests**:
  - Added pytest-regressions[num] as development requirement
  - Added regression tests for `gridr.chain.grid_resampling_chain.basic_grid_resampling_chain` method covering : interpolation methods, multi-channels data, input mask, input grid mask, output mask production, resolution factors, boundary conditions.
  - Added the `test-python-regression` directive to project  `Makefile`
  - Reference regression data must be generated by calling `pytest tests/python/regression` from a reference branch.
  - Data stored in `tests/python/regression/_regression_data` directory
  
- **Grid Resampling**:
  - Extend the pytest `test_grid_resampling.py` to improve its coverage.
