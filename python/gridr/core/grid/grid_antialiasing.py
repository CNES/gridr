...
from gridr.core.filter.lattice import Lattice2d, Domain
from gridr.core.filter.frequential_interp_2d import (
        FrequentialInterpolator2d, ReciprocalCellModel)
...
def compute_antialiasing_filter_from_grid(
        ...
        filter_nrow: int,
        filter_ncol: int,
        filter_cutoff_tanh_slope: Optional[float] = 10.,
        filter_cutoff_shift: float = 0.,
    ...
    working_geometry = Lattice2d(
            v1=V[:,0], v2=V[:,1], domain=Domain.SPATIAL_DOMAIN)
    
    cell_geometry = Lattice2d(
            v1=np.array([1., 0.]), v2=np.array([0., 1.]),
            domain=Domain.SPATIAL_DOMAIN)
    ...
    if case in (ComputeAntialiasingFilterStatus.REQUIRED,
        ...
        filter_interp.compute(
                nrow=filter_nrow, ncol=filter_ncol,
                oversampling_row=1, oversampling_col=1)
