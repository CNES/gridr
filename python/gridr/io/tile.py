...
from gridr.core.utils.array_window import window_check, from_rio_window
...
def read_tile_edges(
    ...
    if window is None:
        ...
        if not window_check(arr, arr_win, axes=None):
            raise ValueError(f"window check fails : check window {arr_win} / "
    ...
    edge_rows = np.empty((edge_row_indices.size, window.width),
            dtype=ds.dtypes[ds_band-1], order='C')
    edge_cols = np.empty((edge_col_indices.size, window.height),
            dtype=ds.dtypes[ds_band-1], order='C')
