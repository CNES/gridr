...
def fft_filtering_oa_strip_chain(
    ...
    if len(chunk_boundaries) == 1:
        ...
        buffer = np.zeros((max(chunk_sizes), ds_in.width),
                dtype=ds_in.profile['dtype'], order='C')
        ...
        for chunk_idx, (lower_line, upper_line) in enumerate(chunk_boundaries):
            ...
            chunk_window = Window.from_slices((lower_line, upper_line),
                    (0, ds_in.width))
            
            logger.debug(f"chunk idx : {chunk_idx}"
                    f"- from line {lower_line} to line {upper_line}")
            
            # Read the data
            arr = ds_in.read(band, window = chunk_window,
                    out=buffer[0:upper_line-lower_line, :])
            
            logger.debug(f"chunk idx : {chunk_idx} "
                    f"- Read array shape : {arr.shape}")
            ...
            logger.debug(f"chunk idx {chunk_idx} "
                    f"- call fft_array_filter with boundary {cboundary}")
            ...
            logger.debug(f"chunk idx {chunk_idx} "
                    f"- current array output shape : {carr_out.shape}")
            ...
            if chunk_idx > 0:
                ...
                oa_dst_window = Window.from_slices(
                        (dst_start_line, dst_stop_line), col_slice_out)
                
                logger.debug(f"chunk idx {chunk_idx}"
                        " - writing top overlapping area")
                logger.debug(f"chunk idx {chunk_idx}"
            ...
            noa_dst_window = Window.from_slices(
                    (dst_start_line, dst_stop_line), col_slice_out)
            
            logger.debug(f"chunk idx {chunk_idx} "
                    "- writing non overlapping area")
            logger.debug(f"chunk idx {chunk_idx} --- non overlapping area src "
                    f"start line : {noa_src_start}")
            logger.debug(f"chunk idx {chunk_idx} --- non overlapping area src "
                    f"stop line (non inc.) : {noa_src_stop}")
            logger.debug(f"chunk idx {chunk_idx} --- non overlapping area dest "
