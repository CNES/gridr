...
use crate::core::gx_grid_resampling::{array1_grid_resampling, GridMeshValidator, NoCheckGridMeshValidator, MaskGridMeshValidator};
    ...
    W: Element + Copy + PartialEq + Default + std::ops::Mul<f64, Output=f64> + Into<f64>,
{
    ...
    let grid_mask_array_view = match grid_mask {
        Some(a_mask_grid) => {
            ...
            let grid_mask_slice = grid_mask_view.as_slice()?; // Récupère le slice immuable
            Some(GxArrayView::new(grid_mask_slice, 1, grid_shape.0, grid_shape.1)) // Adapter nvar, nrow, ncol si besoin
        }
        None => None, 
    };
    
    match grid_mask_flag {
        0 => {
        ...
        1 => {
            ...
            let grid_checker = MaskGridMeshValidator{ mask_view: &mask_view };
        ...
        _ => Err(PyValueError::new_err("Mask mode not implemented")),
    }
}

/// This function calls the generic [`py_array1_grid_resampling`] with `T = f64`, `U = u8`, `V = f64` and `W = f64`.
#[pyfunction]
#[pyo3(signature = (array_in, array_in_shape, grid_row, grid_col, grid_shape, grid_resolution, array_out, array_out_shape, nodata_out, array_in_mask=None, grid_mask=None, array_out_mask=None, grid_win=None))]
