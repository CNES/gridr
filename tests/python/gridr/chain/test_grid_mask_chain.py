...
from gridr.chain.grid_mask_chain import build_grid_mask_chain, build_mask_chain
...
MASK_IN_ARRAY_00 = np.ones(np.prod(MASK_IN_ARRAY_00_SHAPE), dtype=np.uint8).reshape(
    MASK_IN_ARRAY_00_SHAPE
)
# Mask a window in the input array
MASK_IN_ARRAY_00_MASK_WINDOW = [(20, 30), (11, 15)]
MASK_IN_ARRAY_00[
    slice(*MASK_IN_ARRAY_00_MASK_WINDOW[0]), slice(*MASK_IN_ARRAY_00_MASK_WINDOW[1])
] = 0
...
MASK_OUT_ARRAY_00 = np.ones(np.prod(MASK_OUT_ARRAY_00_SHAPE), dtype=np.uint8).reshape(
    MASK_OUT_ARRAY_00_SHAPE
)
...
MASK_OUT_ARRAY_00[
    slice(*MASK_OUT_ARRAY_00_MASK_WINDOW[0]), slice(*MASK_OUT_ARRAY_00_MASK_WINDOW[1])
] = 0
...
class TestGridMaskChain:
    ...
    def test_build_mask_chain_no_mask_in(
        self,
        request,
        data,
        expected,
    ...
    def test_build_grid_mask_chain(
        self,
        request,
        data,
        expected,
        ...
        try:
            ...
            with rasterio.open(
                ...
                "w",
                driver="GTiff",
                dtype=grid_data_in.dtype,
                height=grid_data_in.shape[1],
                width=grid_data_in.shape[2],
                count=2,
            ) as grid_in_ds:
            ...
            with (
                ...
                rasterio.open(
                    grid_out, "w", dtype=computation_dtype, count=2, **open_kwargs
                ) as grid_out_ds,
                rasterio.open(
