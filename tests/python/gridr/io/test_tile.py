...
class TestReadTileEdges:
    ...
    def test_combinations(self, merge, check, window):
        ...
        expected_row_idx = np.array([row_start + i for i in expected_edges(row_start, row_stop, tile_shape[0], merge)])
        expected_col_idx = np.array([col_start + i for i in expected_edges(col_start, col_stop, tile_shape[1], merge)])
